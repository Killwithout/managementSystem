$.fn.bindUpload = function () {
    ...
    this.openWindow = function (files) {
        ...
        _this.find(".off").bind("click", function () {
            ...
            status = [];
            files = null;
            var hiddens = $("#callbackPath2").val();
            if(hiddens != ""){
            	var imgName = hiddens.substring(hiddens.lastIndexOf("/"));
            	var imgPath = "/downFile"+imgName;
            	$("#cjk_img").attr('src',imgPath);
            	$("#cjk_img").attr('style','display:block;padding-left:5px;');
            }
